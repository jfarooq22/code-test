...
export function StaffCard({ member, isSaved, onSave, onRemove }: StaffCardProps) {
    ...
    return (
        ...
        >
            ...
            {/* Save button */}
            <button
                onClick={() => isSaved ? onRemove(member.id) : onSave(member.id)}
                aria-label={
                    isSaved
                        ? `Remove ${member.firstName} from saved list`
                        : `Save ${member.firstName} to on-call list`
                }
                className={`w-full py-2 rounded-lg text-sm font-medium transition-colors ${isSaved
            ...
            >
                {isSaved ? "Remove from On-Call List" : "Save to On-Call List"}
            </button>
        </article>
    )
}
